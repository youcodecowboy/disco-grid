...
export function ZoneBlock({ block, onEdit }: ZoneBlockProps) {
  const name = block.props?.name || block.title || "Zone"
  ...
  const color = block.props?.color || "#10b981"
  ...
  return (
    <div
      className="h-full w-full rounded-lg border-2 border-dashed shadow-sm hover:shadow-md transition-all cursor-pointer p-3 flex flex-col items-center justify-center gap-2 relative"
