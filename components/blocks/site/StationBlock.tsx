...
export function StationBlock({ block, onEdit }: StationBlockProps) {
  const name = block.props?.name || block.title || "Station"
  ...
  const color = block.props?.color || "#06b6d4"
  ...
  return (
    ...
    >
      ...
      {/* Type label */}
      <div className="text-xs text-gray-500">Work Station</div>
    </div>
  )
}
