/**
 * Playbook Generation API Route
 * 
 * Uses LLM to generate playbook automation rules from natural language description
 */
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Get playbook_creation prompt (comprehensive prompt from prompts.ts)
    const promptConfig = getPromptForContext('playbook_creation');
    ...
    console.log('🤖 Generating playbook from:', userMessage.substring(0, 100) + '...');
    ...
    try {
      response = await together.chat.completions.create({
        ...
        temperature: 0.2,
        max_tokens: 12000, // Large playbooks need lots of tokens - prioritize accuracy
      });
    } catch (apiError: any) {
    ...
    try {
      ...
      try {
        ...
        // Check if response was truncated
        if (response.choices[0]?.finish_reason === 'length') {
          throw new Error(`LLM response was truncated (hit max_tokens limit of 12000). The JSON is incomplete. Generated ${response.usage?.completion_tokens || 0} tokens. This playbook is very complex - consider breaking it into smaller playbooks or we can increase max_tokens further.`);
        }
    ...
    console.log(`✅ Generated ${result.plays.length} plays`);
